...
window.Words18Mount.en = function (root, opts) {
  ...
  const STORAGE_KEY = "words18_en_progress_v1";
  const PLAYED_KEY = "words18_en_played_dates";
  const THEME_KEY = "words18_en_theme";
  ...
  function startWord() {
    ...
    el.wordLenBadge.textContent = word.length + " letters";
  ...
  function resolveWord(success) {
    ...
    if (success) {
      ...
      el.feedback.textContent = `Correct! +${points}`;
      el.feedback.className = "feedback good";
      const slots = el.answerSlots.querySelectorAll(".answer-slot");
      slots.forEach((s) => s.classList.add("correct"));
    } else {
      round.log.push({ word, ok: false });
      el.feedback.textContent = `Time's up: ${word}`;
  ...
  function showAd(missedWord, onDone) {
    ...
    el.btnAdClose.textContent = "Playing…";
    ...
    adInterval = setInterval(() => {
      ...
      if (secondsLeft <= 0) {
        ...
        el.btnAdClose.textContent = "Continue";
        finish();
      }
    }, 1000);
  }
  ...
  function refreshArchivePicker() {
    ...
    for (const d of playable) {
      ...
      opt.value = d;
      opt.textContent = `${d} (day #${dayNumberFor(d)})`;
  ...
  function showSummary() {
    ...
    el.summaryTitle.textContent = round.isArchive ? "Archive Day Results" : "Today's Results";
    el.summaryNext.textContent = round.isArchive ? "Choose another day below" : "New words tomorrow";
    ...
    el.btnShare.onclick = () => {
      ...
      const text = `18 Words — Day #${dayNumberFor(round.date)}\n${round.correct}/${ROUND_LENGTHS.length} ✅  •  ${round.score} points\n${grid}\n${location.href}`;
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
          el.btnShare.textContent = "Copied!";
          setTimeout(() => (el.btnShare.textContent = "📋 Copy Result"), 1500);
        });
      }
    };
